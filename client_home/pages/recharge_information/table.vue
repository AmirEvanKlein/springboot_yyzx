<template>
  <view class="page_recharge_information diy_table diy_table_tml" id="recharge_information_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="普通用户" name="ordinary_users">
          <uni-data-select
          placeholder="请选择"
          v-model="query['ordinary_users']"
          :localdata="list_ordinary_users"
      ></uni-data-select>
                    </uni-forms-item>
                    </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/recharge_information/view?" v-show="user_group == '管理员' || $check_action('/recharge_information/table','add') || $check_action('/recharge_information/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','recharge_number')" class="col">
						<view class="view">
                            ...
                            <span>充值号</span>
								...
								<text>
									{{ o["recharge_number"] }}
													...
													<view v-if="1 && $check_field('get','ordinary_users')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>普通用户</span>
                          </view>
								...
								<text>
									{{ get_user_ordinary_users(o['ordinary_users']) }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="1 && $check_field('get','recharge_amount')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>充值金额</span>
                          </view>
								...
								<text>
									{{ o['recharge_amount'] }}
													...
													<view v-if="1 && $check_field('get','recharge_date')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>充值日期</span>
                          </view>
								...
								<text>
									{{ $toTime(o["recharge_date"],"yyyy-MM-dd") }}
								</text>
							</view>
										</view>
					</view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/recharge_information/table','del') || $check_action('/recharge_information/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/recharge_information/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/recharge_information/table','set') || $check_action('/recharge_information/view','set') || $check_action('/recharge_information/view','get')" class="btn btn-primary">详情</navigator>
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/recharge_information/table')" class='btn btn-primary'>支付</a>
	                                <!--跨表按钮-->
                                <a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/recharge_confirmation/view','add')" @click="to_form('/pages/recharge_confirmation/view',o)">
                      <span>充值确认</span>
                    </a>
                        					</view>
				</view>
	...
	export default {
		...
		data() {
			return {
				...
				// 数据
				list: [],
                                        list_ordinary_users: [{value:"",text:"全部"}],
                        // 用户列表
    ...
    return param;
          ...
          async get_list_ordinary_users() {
            ...
            if(json.result && json.result.list){
              json.result.list.map((o) => this.list_ordinary_users.push({value:o.user_id,text:o.nickname + '-' + o.username}));
            }
            else if(json.error){
          ...
          submit_pay(){
            ...
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/recharge_information/table");
                }, 700);
              }else if(res.error){
